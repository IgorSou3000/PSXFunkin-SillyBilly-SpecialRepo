...
#define SPU_MASTER_VOL_L	*((volatile u16*)0x1f801d80)
#define SPU_MASTER_VOL_R	*((volatile u16*)0x1f801d82)
#define SPU_CD_VOL_L		*((volatile u16*)0x1f801db0)
#define SPU_CD_VOL_R		*((volatile u16*)0x1f801db2)
...
static void XA_Init(void)
{
	...
	spu_attr.mask = SPU_COMMON_CDMIX | SPU_COMMON_CDVOLL | SPU_COMMON_CDVOLR | SPU_COMMON_MVOLL | SPU_COMMON_MVOLR;
	spu_attr.cd.mix = SPU_ON;
	spu_attr.cd.volume.left = spu_attr.cd.volume.right = 0x7fff; //Lame magic number
	spu_attr.mvol.left = spu_attr.mvol.right = 0x3fff; //Lame magic number
