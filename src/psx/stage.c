...
static void Stage_LoadMusic(void)
{
	...
	stage.has_countdown = false;

	stage.note_scroll = FIXED_DEC(-1 * 5 * 12,1);
...
void Stage_Load(StageId id, StageDiff difficulty, boolean story)
{
	...
	IO_FindFile(&stage.str_intro_lba, "\\STR\\INTRO.STR;1");
	IO_FindFile(&stage.str_myway_lba, "\\STR\\MYWAY.STR;1");
	...
	if (stage.stage_id == StageId_Bopeebo)
		IO_SeekFile(&stage.str_intro_lba);
...
void Stage_Tick(void)
{
	...
	if (stage.stage_id == StageId_Bopeebo && stage.song_step == 3492 && stage.flag & STAGE_FLAG_JUST_STEP && !stage.movie_is_playing)
	{
		stage.audio_start_pos = 338;
		Str_PlayFile(&stage.str_myway_lba);
			...
			if (!(stage.flag & STAGE_FLAG_PAUSED))
			{
				if (stage.note_scroll < 0)
				{
					...
					if (stage.song_time >= 0)
					{
						...
						if (stage.stage_id == StageId_Bopeebo)
							Str_PlayFile(&stage.str_intro_lba);
						Audio_PlayXA_Track(stage.stage_def->track, 0x40, stage.stage_def->channel, false, stage.audio_start_pos);
						Audio_WaitPlayXA();
